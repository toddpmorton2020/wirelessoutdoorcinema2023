...
const HeroSection = ({ page = '' }) => {
    const { HeroImg, TabletHeroImg, MobileHeroImg } = useStaticQuery(
        graphql`
          query {
            HeroImg: file(relativePath: {eq: "how-to-build-an-outdoor-movie-theater-1920px-x-680px-winter.jpg"}) {
              childImageSharp {
                gatsbyImageData(quality: 100, layout: CONSTRAINED)
              }
            } 
            TabletHeroImg: file(relativePath: {eq: "how-to-build-an-outdoor-movie-theater-1920px-x-680px-winter.jpg"}) {
                childImageSharp {
                  gatsbyImageData(quality: 100, layout: CONSTRAINED)
                }
              } 
            MobileHeroImg: file(relativePath: {eq: "mobile-how-to-build-an-outdoor-movie-theater-480x320-winter.jpg"}) {
                childImageSharp {
                  gatsbyImageData(quality: 100, layout: CONSTRAINED)
                }
              }           
        ...
        <>
            ...
            <div className={`bg-black relative text-white herosection ${page !== 'home' ? 'sm:top-0 sm:-mt-12' : ''}`}>
                <div className="hidden lg:block">
                    <GatsbyImage image={getImage(HeroImg)} alt="How to build an outdoor movie theater. Outdoor theater setup in front of a lake in the winter." loading="eager"
                        className="mx-auto w-full max-w-2400 pt-10 pb-12" />
                </div>
                <div className="lg:hidden">
                    <div className="sm:block hidden">
                        <GatsbyImage image={getImage(TabletHeroImg)} alt="How to build an outdoor movie theater. Outdoor theater setup in front of a lake in the winter."
                            className="mx-auto w-full" />
                    </div>
                    <div className="sm:hidden">
                        <GatsbyImage image={getImage(MobileHeroImg)} alt="How to build an outdoor movie theater. Outdoor theater setup in front of a lake in the winter."
                            className="mx-auto w-full" />
                    </div>                   
                </div>
                    ...
                    <div className="lg:absolute xl:mt-16 2xl:pt-18 sm:top-0">
                        <div className="relative lg:w-3/5 xl:w-2/4 2xl:w-3/6 z-20 xl:mt-0 lg:mt-20 mt-8 xl:mt-0">
                            <p className="font-semibold text-xl sm:text-2xl md:text-4xl mb-0 max-w-full">
                                Get the only <span className="text-orange-1000">professional DIY</span> guide on
                            </p>
